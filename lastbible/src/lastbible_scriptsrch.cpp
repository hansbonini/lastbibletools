...
//const static int searchStart = 0x6C000;
//const static int searchEnd   = 0x78000;
//const static int searchStart = 0x08000;
//const static int searchEnd   = 0x10000;
const static int searchStart = 0x10000;
const static int searchEnd   = 0x12500;
...
//    ofs << resultStr;
    ...
    // this shouldn't occur outside of the [char] op, and we already recognize that
    // separately
    else if (result.id == 0x00) {
      return -(ifs.tell() - start);
    }
    else if (result.id == op_terminator) {
      return (ifs.tell() - start);
    }
  }
}
...
//      ifs.seekoff(-size);
    ...
    // no more than 1 string at a time
    else {
      endpos = (pos + size);
      ++count;
      break;
...
//    std::cerr << std::hex << pos << " " << (ifs.tell() - pos) << std::endl;
    ofs << "  dumpStringSet(ifs, ofs, tablestd, "
...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    cout << "Last Bible string searcher" << endl;
